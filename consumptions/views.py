...
CATEGORY_MAPPER = {'채소':1, '샐러드':1, '나물':1, '과일':2, '과실':2, '두부':3, '콩/두부':3, '두류':3, '두유':3, '메주':3, '된장':3, 
                   ...
                   '우유':9, '치즈':9, '유제품':9, '발효유':9, '가공유':9, '가공두유':9, '보충제':0}
...
## TODO : admin - 사용자별 식단 수정(기존api)/삭제(기존api)/입력(새로운 api)
# GET은 AdminDayView 사용
# PACTH/DELETE는 api를 새로 만들어야 하나 기존 것을 그대로 사용해도 되나? (DONE)
# 'food/<int:pk>/', views.FoodConsumptionUpdateAPIView.as_view() : 이 api가 본인 작성한 것 외에 admin이나 staff도 사용가능한지 체크 필요 -> IsOwnerorAdmin(BasePermission)에 staff 추가
